...
contract('ERC20',function(accounts){
    ...
    it('initializes the contract with correct values',function(){
        return ERC20.deployed().then(function(instance){
            ...
            assert.equal(name,"ERC20",'has the correst name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol,"ERC",'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard,"ERC20 v1.0",'has the correct standard');
        });
    });

    it('allocatec the initial supply upon deployment',function(){
        return ERC20.deployed().then(function(instance){
            tokenInstance = instance;
    ...
    it('transfers token ownership',function(){
        return ERC20.deployed().then(function(instance){
          ...
          assert.equal(success, true, 'it returns true');
            ...
            assert.equal(balance.toNumber(),250000,'ads the amount to recirving account');
            return tokenInstance.balanceOf(accounts[0]);
        }).then(function(balance){
    ...
    it('approves tokens for delicated transfer', function(){
        return ERC20.deployed().then(function(instance){
            tokenInstance = instance;
            ...
            assert.equal(allowance.toNumber(),100,'atores the allowance');
        });
    });
    ...
    it('handles deligated transfer',function(){
        return ERC20.deployed().then(function(instance){
            ...
            //try sending something larger than the senders balance
